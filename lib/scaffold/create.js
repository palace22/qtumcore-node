...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'qtumcore-lib': 'https://github.com/palace22/qtumcore-lib.git#master',
    'qtumcore-node': 'https://github.com/palace22/qtumcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/qtumcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the qtumcore-node directory and configuration
